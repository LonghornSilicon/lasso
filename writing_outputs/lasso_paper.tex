...
real model attention snapshots. A binary \{INT4, INT8\} precision
scheme achieves 2.93$\times$ decode speedup at 4.80 avg-bits on
SmolLM-1.7B, outperforming four-precision \{2,4,8,16\} schemes by
43\% throughput at equal accuracy.
\end{abstract}
...
targets an open-source ASIC implementation in the SKY130 process or
employs a simulation-driven pre-RTL methodology to surface and fix
data-path issues before committing to hardware.
...
\begin{itemize}
  ...
  \item A complete \textbf{SKY130 golden model} with 158 tests and a
        FPGA validation plan for Xilinx Zynq UltraScale+ (ZCU102/ZCU104).
\end{itemize}
...
\textbf{KV Eviction and Compression.}
H2O~\cite{h2o2023} evicts tokens by cumulative attention score,
a proxy for importance. SnapKV~\cite{snapkv2024} improves upon H2O
with observation-window calibration and per-layer thresholds.
StreamingLLM~\cite{streamingllm2023} identifies attention sinks and
retains only them plus a local window. \lasso\ combines sink retention
(from StreamingLLM) with dynamic threshold calibration (from SnapKV)
and validates the $C_t$/$H_t$ signals empirically against effect-size
analysis. GEAR~\cite{gear2024} applies 4-bit quantization with
low-rank residual correction; \lasso\ drops the correction path in
favor of WHT rotation, which achieves equivalent accuracy with less
area.

\textbf{Quantized Attention Hardware.}
SageAttention~\cite{sageattention2024} achieves 2.7$\times$ speedup
over FlashAttention-2 using INT8 $\texttt{mma.u8.u8.s32}$ Tensor
Core instructions, keeping softmax weights in FP16. Our INT64
accumulator finding echoes the INT32 choice in SageAttention for INT8
inputs: larger bit-widths require wider accumulators.
INT-FlashAttention~\cite{intflashattention2024} quantizes entire
attention computation to INT8 with per-row FP32 running maxima;
it confirms FP32 (or INT64) is necessary to prevent softmax overflow.

\textbf{Domain-Specific Accelerators.}
Titanus~\cite{titanus2025} proposes a cascade pruning-quantization
(CPQ) pipeline in computing-in-memory chiplets, reporting 159.9$\times$
energy efficiency vs.\ A100 for long-context inference.
Kelle~\cite{kelle2025} demonstrates 3.9$\times$ speedup and
4.5$\times$ energy improvement by co-designing KV eviction policy
(AERP) with embedded eDRAM at 84.8\,pJ/B bandwidth. \lasso\ differs
from both in targeting an open-source ASIC process (SKY130) and
providing a complete simulation-to-tapeout methodology.

\textbf{Rotation-Based Quantization.}
QuIP\#~\cite{quip2024} applies randomized Hadamard transforms for
weight quantization to suppress outliers. TurboQuant~\cite{turboquant2026}
extends this to KV activations using WHT (stage~1) and Lloyd-Max
codebook (stage~2), reporting +2\,pp on HellaSwag and +3\,pp on
ARC-Challenge at identical 5.05\,bits/token vs.\ scalar quantization.
\lasso\ implements TurboQuant stages~1--2 in hardware at ${\approx}0.055\,\text{mm}^2$.

\textbf{Integer-Domain Softmax.}
IntAttention~\cite{intattention2025} implements a fully integer
attention pipeline on Armv8 CPUs without retraining, using IndexSoftmax
(32-entry LUT) to avoid floating-point exponentials. BAPS~\cite{baps2026}
uses 8-bit floating-point block-aware precision rescaling for softmax,
halving bandwidth. These approaches are relevant to future LACU
extensions beyond the v1 scope.
...
The design targets SKY130 130\,nm at ${\le}7.13\,\text{mm}^2$ with
3.15\,mm$^2$ headroom. FPGA prototyping on Xilinx Zynq UltraScale+
...
%   All others marked below. Use DOI resolver or arXiv API before submitting.
%------------------------------------------------------------------
